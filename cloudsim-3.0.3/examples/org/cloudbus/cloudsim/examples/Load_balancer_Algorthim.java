...
public class Load_balancer_Algorthim
{
...
private static List<Vm> createVM(int userId, int vms, int idShift) {
	...
	Vm[] vm = new Vm[vms];
		...
		list.add(vm[4]);
		vm[5] = new Vm(5, userId,20, pesNumber, ram, bw, 1000, vmm, new CloudletSchedulerTimeShared());//creates a new vm characteristics object
		list.add(vm[5]);
	return list;
}
...
public static void main(String[] args) {
	...
	try {
		...
		vmList = createVM(brokerId, 6, 0); //creating 6 vms
		...
		/*Log.printLine("Debt of each user to each Datacenter");
		datacenter0.printDebts();
		datacenter1.printDebts();
		datacenter1.printDebts();*/
		

		Log.printLine(Load_balancer_Algorthim.class.getName() + " finished!");
	}
	catch (Exception e)
	{
		e.printStackTrace();
